...
function openPopup(genre, description, subgenre, examples, embedID) {
    genreTitle.textContent = genre;
    genreBody.textContent = description;
    genreSubgenre.innerHTML = subgenre;
    genreExamples.innerHTML = examples;
    genreEmbedLink.innerHTML = createIframeHTML(embedID);
...
genreLinks.forEach(link => {
    link.addEventListener('click', function(event) {
        ...
        const genreEmbedID = this.getAttribute('data-embed');

        openPopup(genreName, genreDesc, genreSub, genreExample, genreEmbedID);
    });
});
